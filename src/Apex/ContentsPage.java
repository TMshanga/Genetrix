...
import Apex.PageViewer.CustomTab;
...
public class ContentsPage {
	...
	public VBox getContentsPage() {
		...
		// Title
		Text text = new Text("ℂ𝕠𝕟𝕥𝕖𝕟𝕥𝕤 ℙ𝕒𝕘𝕖");
		...
		stackPane.getChildren().addAll(text, new Separator(), new MenuBar(pageMenu), tree);
		return stackPane;
	}
	...
	private void depthFirstAssembily(Tree.Node<PageInterface> pageNode, ArrayList<Tree.Node<PageInterface>> visited,
	...
	class cutAction implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent event) {
			...
			Tree.Node<PageInterface> node = (Tree.Node<PageInterface>) Main.currentProject.pageTree
					.getNode(item.internalAddress);
